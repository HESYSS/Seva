const addButtonSquare = document.getElementById('addButtonSquare');
const deleteButtonSquare = document.getElementById('deleteButtonSquare');
    
    
    
    let isElementDraggingSquare = false;
    let dexteritySquare = 0;



    
    function dexteritySquareAdd(){
      dexteritySquare = document.getElementById('dexteritySquareAdd').value;
    }

    // Функция для добавления нового элемента в список InitiativeList
    

    addButtonSquare.addEventListener('click', () => {
      const newElement = document.createElement('div');
      const StateElement = document.createElement('div');
      newElement.classList.add('square');
      StateElement.classList.add('square');
      ...
      const Initiative = Math.floor(Math.random() * 20) + 1 + Number(dexteritySquare);
      StateElement.Initiative = Initiative;


      newElement.Hp = hp('squarehp')
      ...
      const addButtonSquareRect = addButtonSquare.getBoundingClientRect();

      newElement.style.position = 'absolute';
      newElement.style.left = addButtonSquareRect.left + 'px';
      newElement.style.top = addButtonSquareRect.bottom + 'px';
    ...
    deleteButtonSquare.addEventListener('click', () => {
      if (selectedElement) {
        ...
        deleteButtonSquare.setAttribute('disabled', 'true');
      }
    });
